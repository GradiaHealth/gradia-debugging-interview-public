...
import { findRedirectByURL, findRedirectByCode, insertRedirect } from "./apiHelper";
...
export async function lambdaHandler(event: any): Promise<ShortURLResp> {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (options.hasOwnProperty("originalUrl")) {
      ...
      findRedirectByCode(options.originalUrl)
        .then((foundResp) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    do {
      if (i > 10) {
        ...
        reject("Couldn't find a valid code - probably a database error");
        return;
      }
      nanoidId = nanoid();
...
function codeExists(code: string): Promise<boolean> {
  console.log("Checking if we already have a redirect set up for code: ", code);
  return new Promise((resolve, reject) => {
    findRedirectByURL(code)
      .then(() => resolve(true))
      .catch((err) => {
        if (err) {
          console.error(err);
